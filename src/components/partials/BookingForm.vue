  ...
  <section class="book-input">
    <form @submit="checkInput">
      <div class="select-wrapper">
        <b-field label="Välj en kategori">
          <b-select v-model="booking.category" placeholder="-" id="category-select" use-html5-validation required>
      ...
      <b-field>
        <b-input placeholder="Telefonnummer" type="tel" min="9" max="10" v-model="booking.phone" use-html5-validation
          required>
        </b-input>
      </b-field>
      ...
      <time-picker v-on:emitSelectedTime="booking.time = $event" :disabled="timePicker.isDisabled" :unselectable="timePicker.bookedTimes" />
    ...
    <modal v-show="messageModal.isVisible" @close="messageModal.isVisible = false">
      ...
      <div class="modal-error-message" slot="body">
        ...
        <div class="btn-wrapper">
          <button type="button" :class="messageModal.btnClass" @click="messageModal.btnAction">{{
            messageModal.btnTitle }}</button>
        </div>
      </div>
    </modal>
    ...
    <modal v-show="confirmModal.isVisible" @close="confirmModal.isVisible = false">
      ...
      <div class="modal-confirm-booking" slot="body">
        <ul class="booking-list">
          <li>
            <label class="label title-label">Förnamn: </label>
            <p>{{ this.booking.firstName }}</p>
          </li>
          <li>
            <label class="label title-label">Efternamn: </label>
            <p>{{ this.booking.lastName }}</p>
          </li>
          <li>
            <label class="label title-label">Email: </label>
            <p>{{ this.booking.email }}</p>
          </li>
          <li>
            <label class="label title-label">Telefon: </label>
            <p>{{ this.booking.phone }}</p>
          </li>
          <li>
            <label class="label title-label">Kategori: </label>
            <p>{{ this.booking.category }}</p>
          </li>
          <li>
            <label class="label title-label">Beskrivning: </label>
            <p>{{ this.booking.description }}</p>
          </li>
          <li>
            <label class="label title-label">Datum: </label>
            <p>{{ this.booking.date }}</p>
          </li>
          <li>
            <label class="label title-label">Tid: </label>
            <p>{{ this.booking.time }}</p>
          </li>
        </ul>
				{{displayStatus}}
        <div class="btn-wrapper">
          <button type="button" class="btn-primary" v-on:click.prevent="emitBooking">Skicka</button>
...
<script>
  import {
    fetchBookingsByDate
  } from '@/functions/recurringFetch.js';
  ...
  export default {
    ...
    computed: {
      displayStatus: function () {
        if (this.$props.bookingStatus && this.$props.bookingStatus !== '') {

					/* Hide confirm booking and display success message in modal */
					this.confirmModal.isVisible = false;
					
					if (this.$props.bookingStatus.success === true) {
							this.messageModal.isVisible = true;
							this.messageModal.title = 'Yes!';
							this.messageModal.message = this.$props.bookingStatus.message;
							this.messageModal.btnClass =  'btn btn-primary';
							this.messageModal.btnTitle = 'Okej';

						} else {
						/* Hide confirm booking and display error message in modal */
						this.confirmModal.isVisible = false;

						this.messageModal = {
							isVisible: true,
							title: 'No!',
							message: this.$props.bookingStatus.message,
							btnClass: 'btn btn-secondary',
							btnTitle: 'Försök igen'
						}
					}
        } else {
					return;
    ...
    methods: {
      ...
      closeModal: function () {
        this.messageModal.isVisible = false;
      },
